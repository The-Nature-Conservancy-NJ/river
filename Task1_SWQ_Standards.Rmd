...
days_over_table <- temp_count %>%
  ...
  kbl(caption = "Total number of days with temperature readings over the Trout Maintenance threshold. Asterisks indicate years with less complete data during the warmest period (Jun-Aug). * = less than 80% coverage; ** = less than 50% coverage.") %>%
  kable_classic(full_width = F, html_font = "Cambria")
...
yearf_mat <- matrix(data = rep(1:9, n_sites), nrow = n_sites, ncol = n_years, byrow = T)
...
year6_mat <- matrix(data = c(rep(0,5*n_sites), 
                             rep(1,n_sites),
                             rep(0, length(TMex_mat)-6*n_sites)),
                    ncol = n_years)
year7_mat <- matrix(data = c(rep(0,6*n_sites), 
                             rep(1,n_sites),
                             rep(0, length(TMex_mat)-7*n_sites)),
                    ncol = n_years)
year8_mat <- matrix(data = c(rep(0,7*n_sites), 
                             rep(1,n_sites),
                             rep(0, length(TMex_mat)-8*n_sites)),
                    ncol = n_years)
year9_mat <- matrix(data = c(rep(0,8*n_sites), 
                             rep(1,n_sites),
                             rep(0, length(TMex_mat)-9*n_sites)),
                    ncol = n_years)
year10_mat <- matrix(data = c(rep(0,9*n_sites), 
                             rep(1,n_sites),
                             rep(0, length(TMex_mat)-10*n_sites)),
                    ncol = n_years)
...
year6_mat <- matrix(data = c(rep(0,5*n_sites), 
                             rep(1,n_sites),
                             rep(0,length(TMexDO_mat)-6*n_sites)),
                    ncol = n_years)
year7_mat <- matrix(data = c(rep(0,6*n_sites), 
                             rep(1,n_sites),
                             rep(0,length(TMexDO_mat)-7*n_sites)),
                    ncol = n_years)
year8_mat <- matrix(data = c(rep(0,7*n_sites), 
                             rep(1,n_sites),
                             rep(0,length(TMexDO_mat)-8*n_sites)),
                    ncol = n_years)
year9_mat <- matrix(data = c(rep(0,8*n_sites), 
                             rep(1,n_sites),
                             rep(0,length(TMexDO_mat)-9*n_sites)),
                    ncol = n_years)
year10_mat <- matrix(data = c(rep(0,9*n_sites), 
                             rep(1,n_sites),
                             rep(0,length(TMexDO_mat)-10*n_sites)),
                    ncol = n_years)
...
modsum <- jags_mod$summary %>%
  ...
  mutate(parameter = row.names(jags_mod$summary)) %>%
  filter(grepl(parameter, pattern = "theta")) %>%
  mutate(site = rep(unique(tstat$site), 10),
         year = sort(rep(2016:maxyr, numsites))) %>%
  left_join(tstat_plot) %>%
  filter(is.na(TMex_n)==F) %>%
  bind_rows(river_modsum) %>%
...
Results of Model D.
#max(DO_count$year) was listed as do_count, corrected for code to run
```{r}
