...
# Task 2 description: Site specific water quality conditions important to us other than SWQS
...
tsum %>%
  ...
  geom_errorbar(aes(x = day, ymin = min_temp, ymax = max_temp),
                width = 0, linewidth = .3, color = "firebrick",
                alpha = 0.4) +  
  ...
  geom_errorbar(aes(x = mid, ymin = mean_min, ymax = mean_max),
                width = 0, linewidth = .5, color = "black",
...
```{r}
# make list of sites to plot
sites <- unique(tdata$site)

# format data for min/max ribbon
tsum_envelope <- tdata %>%
  group_by(site, day) %>%
...
tsum_month %>%
  ...
  geom_ribbon(aes(x = day, ymax = multi_max,
              ...
              fill = "gray", alpha = 0.5) +
  geom_line(aes(x = mid, y = mean_mean, color = yearf), linewidth = .5,
             shape = 16,
...
dosum %>%
  ...
  geom_errorbar(aes(x = day, ymin = min_do, ymax = max_do),
                width = 0, linewidth = .3, color = "dodgerblue",
                alpha = 0.4) +  
...
Only include sites accessible by migratory fish.
```{r}
# Create list of sites (excluding those not accessible)
sites <- unique(tdata$site)
sites <- sites[sites %notin% exclude]

# get the year before the overall max year for the study
# this is needed to properly set up the year factor for plotting
max_y_b4 <- max(tdata$year) - 1
...
# make a (repeating) vector of seasons to add to annotate df
#   (repeating makes it flexible to accomodate future years)
seas2_vector <- 
  rep(c("Migrate (S)", "Spawn", "Larvae", "Juvenile","Migrate (F)"),
      4)

# same for annotations
ann_vector <- rep(c("Migration (15 April - 15 May)",
              "Spawning (16-31 May)\nOptimal egg dev.: 17 C",
              "Larvae (1-30 June)\nRequire: 16-26 C",
              "Juvenile (1 July - 15 Sept.)\nRequire: 16-26 C",
              "Migration (16 Sept. - 30 Nov.)"),4)
              
# make vector of years for annotation plot and count them
year_vector <- c(rep(max_y,5),2016:max_y_b4)
yv_len <- length(year_vector)

# create a dataframe to map the text annotations
annotate_df <- data.frame(yearf = as.factor(year_vector),
                          mean_mean = 6,
                          season2 = as.factor(seas2_vector[1:yv_len]),
      ann = ann_vector[1:yv_len]
      ) %>%
  filter(yearf == max_y) %>%
...
# all of that was required to make sure all elements of factors
#   year and season2 appear in the plot where appropriate,
...
rm(plot_data, annotate_df, max_y, color_vector_df, ann_vector,
   max_y_b4, seas2_vector, year_vector, yv_len)
```
...
Warning: very long run time (& juv run uses lots of RAM). All outputs from models and bootstrapping were saved as .rds files in the "statistical_analysis" subfolder. So if you do not need to rerun them, you can skip to section 2.12 (Plot G) and it will still work.
```{r}
