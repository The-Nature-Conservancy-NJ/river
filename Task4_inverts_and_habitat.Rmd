...
Run the first section ("chunk") of this Rmd document (section 4.0). That will load all libraries, functions, and data. After that, you can scroll down to the plot or table you wish to make or the model you wish to run and run just that section. Plots, tables and models are labeled with letters (A-Z) and correspond to those in the "Task 4" sub-folder in the "output" folder. See the README file in that sub-folder for figure captions and information.

The following text is a general description of Task 4.
...
# 4.0. Load libraries and read in data
```{r}
library(readxl)
library(dplyr)
library(ggplot2)
...
inv25 <- future_macro("2025")
inv26 <- future_macro("2026")
...
hab25 <- future_hab("2025")
hab26 <- future_hab("2026")
...
```
# 4.1. Plot A: average macroinvertebrate index by site/year
```{r}
...
rm(inv.plot)
```
# 4.2. Plot B: data visualization of macroinvertebrate index components
Scatterplot matrix to inspect data. Code adapte from:
...
rm(g)
```
# 4.3. Plot C: Spiderweb plot of macroinvertebrate index components
Code adapted from: https://r-graph-gallery.com/142-basic-radar-chart.html
```{r}
# format data
inv_radar <- inv %>%
  select(c(1, 3:9)) %>%
...
```
# 4.4. Plot D: macroinvertebrate index over time (separate plots by site)
```{r}
# for testing
...
plot_data %>%
  ...
  scale_x_continuous(breaks = 2016:2050) +
...
```
# 4.5. Plot E: macroinvertebrate index over time at Columbia Dam sites
```{r}
...
columbia_macro_data %>%
  ...
  ggsave(here(task_path, 
         ...
         width = 6, height = 4, dpi = 400)
```
# 4.6. Plot F: macroinvertebrate index over time at Paulina Dam sites
```{r}
...
```
# 4.7. Plot G: dot plot of overall index average habitat score by site/year
```{r}
# create a vector of colors for Optimal -> Poor
color_vector <- c("olivedrab",
            "olivedrab2",
            wes_palettes$BottleRocket2[1:2])
...
ggsave(here(task_path,
       ...
       height = 4, width = 4.5, dpi = 400)
```
# 4.8. Plot H: macroinvertebrate index vs. habitat score
```{r}
...
```
# 4.9. Plot I: data visualization of habitat score components
...
rm(g)
```
# 4.10. Plot J: Spiderweb plot of habitat score components
```{r}
# format data
hab_radar <- hab %>%
...
```
# 4.11. Plot K: habitat score over time (separate plots by site)
```{r}
...
```
# 4.12. Plot L: habitat & macroinvertebrate index over time 
...
# set x value to place Optimal-Poor annotations
options(digits = 9)
ann_loc <- max(plot_data$year)- 0.5
...
ggplot() +
  ...
  geom_line(aes(x = year, y = index), lty = 2, 
            color = color_vector[1],
             alpha = 0.75,
            data = topline) +
  geom_text(aes(x = ann_loc, y = index+2.5, label = rating),
            data = topline, size = 2,
            color = co[1], hjust = .5,
            alpha = .75) +
  # mid threshold line
...
```
# 4.13. Plot M: mean habitat & macroinvertebrate index over time (all sites)
...
ggsave(here(task_path, 
         ...
         width = 6, height = 4, dpi = 400)
```
# 4.14. Plot N: macroinvertebrate index vs. habitat score + temperature
...
ggsave(here(task_path,
       ...
       height = 4, width = 5, dpi = 400)
```
# 4.15. Plot O: macroinvertebrate index vs. habitat score + DO
...
ggsave(here(task_path,
       ...
       height = 4, width = 5, dpi = 400)
```
# 4.16. Model A: change in average habitat quality score over time
See Model A text file in the Task 4 "statistical_analysis" folder.
```{r}
...
```
# 4.17. Model B: change in average macroinvertebrate score over time
See Model B text file in the Task 4 "statistical_analysis" folder.
```{r}
# format habitat data, averaging within site/year 
# and keeping only sites with >= 4 years of data
inv_data <- inv %>%
...
```
# 4.18. Model C: GLM of mean invertebrate index vs. mean July temp & DO
See Model C text file in the Task 4 "statistical_analysis" folder.
```{r}
...
```
# 4.19. Plot P: mean invertebrate index vs. mean July DO
```{r}
color_vector <- c("olivedrab",
            "olivedrab2",
            wes_palettes$BottleRocket2[1:2])
...
```
# 4.20. Plot Q: macroinvertebrate index over time at tree planting site
sites TNC_4, 5, and 8
```{r}
...
```
# 4.21. Plot R: macroinvertebrate index over time at hyper humus
site TNC_2
```{r}
sites <- c("TNC_2")
